...
(function ($) {
    ...
    /*-----------------------
    	02. Counter Up
    -------------------------*/
    $('.tgl-ios').on("click", function(){
        const checkbox = $("input#cb2");
        const checked = checkbox[0].checked
        const basic = $('#basic-price')
        const basicAnnee = basic.attr("data-annee")
        const basicMois = basic.attr("data-mois")
        
        const business = $('#business-price')
        const businessAnnee = business.attr("data-annee")
        const businessMois = business.attr("data-mois")

        const advanced = $('#advanced-price')
        const advancedAnnee = advanced.attr("data-annee")
        const advancedMois = advanced.attr("data-mois")
        if (checked) {
            basic.html(basicAnnee)
            business.html(businessAnnee)
            advanced.html(advancedAnnee)
        }else{
            basic.html(basicMois)
            business.html(businessMois)
            advanced.html(advancedMois)
        }
    });
    $('.count').counterUp({
        delay: 10,
